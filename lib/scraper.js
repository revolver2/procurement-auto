...
async function scrapeBons(maxItems = 50, onProgress) {
  ...
  try {
    ...
    // Specific search terms — meaningful enough to avoid noise, but not multi-word phrases
    // Excludes generic words like "fourniture", "metal", "abri" that match everything
    const SPECIFIC_TERMS = [
      'aluminium', 'inox', 'metallique', 'alucobond',
      'serrurerie', 'ferronnerie', 'vitrage', 'pergola',
      'charpente', 'menuiserie', 'garde-corps', 'cloison',
      'panneaux sandwich', 'escalier metallique', 'rideau metallique',
      'faux plafond metallique', 'structure metallique', 'facade aluminium',
    ]
    const searchTerms = [...new Set(SPECIFIC_TERMS)].slice(0, 20)
    ...
    for (const term of searchTerms) {
      ...
      try {
        ...
        for (const card of cards) {
          ...
          // Quick keyword check on card title before fetching details
          if (!bonMatchesKeywords({ title: card.title, buyer: card.buyer, description: '', category: '', naturePrestation: '' })) {
            // Still add to seenIds to avoid re-checking, but skip
            continue
          }
          ...
          // Final keyword check with all fields including details
          if (bonMatchesKeywords(bon)) {
            bons.push(bon)
            log(`  ✅ Bon retenu: ${titleFinal.substring(0, 55)}`)
          } else {
            log(`  ⛔ Hors mots-clés: ${titleFinal.substring(0, 55)}`)
          }
        }
      } catch (e) {
