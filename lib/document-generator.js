...
function buildSourceBlock(bon, options = {}) {
  return {
    bcId:               bon.id || '—',
    buyer:              bon.buyer || '—',
    attachmentAnalyzed: options.attachmentAnalyzed || options.primaryAvisName || 'AVIS officiel',
    analysisDate:       new Date().toISOString().split('T')[0],
    aiEngine:           options.aiEngine || 'Gemini 2.5 Flash / Règles locales',
    statement:          "Analyse basée uniquement sur l'AVIS joint officiel. Toute information absente est signalée explicitement.",
  }
}
 ...
 */
async function generate(bon, analysis, documentType, sourceOptions = {}) {
  const prompts = {
  ...
  return {
    type:        documentType,
    data,
    _source:     buildSourceBlock(bon, sourceOptions),
 ...
 */
async function generateAll(bon, analysis, sourceOptions = {}) {
  const types = ['bordereau', 'devis', 'rfq', 'plan', 'checklist']
  const results = {}
  const source = buildSourceBlock(bon, sourceOptions)
  for (const type of types) {
    try {
      const doc = await generate(bon, analysis, type, sourceOptions)
      results[type] = doc.data
      results[`${type}_source`] = source
    }
...
module.exports = { generate, generateAll, buildSourceBlock }
