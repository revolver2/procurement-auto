...
async function orchestrate(bonId, options = {}) {
  ...
  // Return AI cache if exists, valid hash, and not forced
  if (!forceRefresh) {
    const cached = getOrchestration(bonId)
    if (cached) {
      // Check per-bon AI cache for fresher result
  ...
  try {
    ...
    results.attachmentSpec  = spec || null
    results.analysisSource  = bon.officialText ? 'official_attachment_only' : 'metadata_only'

    results.provider   = 'gemini'
    results.sourceHash = hash
