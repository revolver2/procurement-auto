...
async function orchestrate(bonId, options = {}) {
  ...
  try {
    ...
    log.push(`✅ Analyse complète: ${master.materials?.length||0} matériaux, ${master.risks?.length||0} risques, ${master.bordereau?.length||0} lignes bordereau`)

    const results = {
      bonId,
      ...master,
      // Ensure these are always top-level for the normalizer
      profitabilityScore: master.executive?.profitabilityScore ?? 50,
      urgencyScore:       master.executive?.urgencyScore       ?? 50,
      winningProbability: master.executive?.winningProbability ?? 'Modérée',
    ...
    // Update bon in DB with key scores
