...
async function extractPDF(filePath) {
  // pdf-parse v2 exports a class { PDFParse }, not a default function
  let PDFParse
  try {
    const mod = require('pdf-parse')
    PDFParse = mod.PDFParse
    if (typeof PDFParse !== 'function') throw new Error('PDFParse not found in module exports')
  } catch (e) {
    return _pdfFallback(filePath, `pdf-parse non disponible: ${e.message}`, 0)
  }

  // Verify file is a real PDF before parsing
  let isPdf = false
  try {
  ...
  if (!isPdf) {
    return { text: '', textExtracted: false, requiresOCR: false, error: 'Fichier non-PDF invalide' }
  }

  let pageCount = 0
  try {
    // pdf-parse v2 API: new PDFParse({ url: absolutePath }), load(), getText()
    const parser = new PDFParse({ verbosity: 0, url: filePath })
    await parser.load()
    const result = await parser.getText()

    pageCount = result.total || result.pages?.length || 0
    // Strip pdf-parse page markers like "-- 1 of 5 --"
...
async function _pdfFallback(filePath, reason, pageCount) {
  ...
  return {
    ...
    requiresOCR:   true,
    pageCount,
    error:         reason + (process.env.GEMINI_API_KEY ? ' (Gemini OCR échoué)' : ' — ajoutez GEMINI_API_KEY dans Render pour activer l\'OCR'),
  }
}
...
async function extractZip(zipPath) {
  ...
  const sourceZipName = path.basename(zipPath)
  ...
  try {
    ...
    let bestResult = {
      text: '', textExtracted: false,
      error: 'Aucun fichier exploitable dans le ZIP', requiresOCR: false,
    ...
    for (const { entry } of scored) {
      ...
      // Attach source info: the PDF name (not the ZIP name)
      result.sourceFile    = path.basename(name)  // PDF name inside ZIP
      result.sourceZip     = sourceZipName
      result.extractedPath = outPath
    ...
    return { text: '', textExtracted: false, error: `Erreur ZIP: ${e.message}`, requiresOCR: false, sourceZip: sourceZipName }
  }
}
